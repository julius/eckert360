(function() {
    ...
    function smoothViewUpdate() {
        ...
        var params = {
            yaw: view.yaw() + (viewTargetParams.yaw - view.yaw()) * 0.001 * deltaTime,
            pitch: view.pitch() + (viewTargetParams.pitch - view.pitch()) * 0.001 * deltaTime
        };
    ...
    var scenes = data.scenes.map(function(data) {
        ...
        var limiter = CustomView.limit.traditional(data.faceSize, (110 * Math.PI) / 180, (110 * Math.PI) / 180);
        ...
        view.updateWithControlParameters = function(parameters) {
            var vfov = this._fov;
            var hfov = Marzipano.util.convertFov.vtoh(vfov, this._width, this._height);
            if (isNaN(hfov)) {
                hfov = vfov;
            }
            ...
            viewTargetParams = {
                yaw:
                    ...
                    parameters.yaw,
                pitch:
                    viewTargetParams.pitch + parameters.axisScaledY * vfov + parameters.y * 2 * hfov + parameters.pitch
            };
    ...
    document.querySelector('#imageNav .imageNavBackdrop').addEventListener('click', function() {
        document.getElementById('imageNav').classList.remove('active');
    });
    document.querySelector('#imageNav .close').addEventListener('click', function() {
        document.getElementById('imageNav').classList.remove('active');
    });
    document.querySelectorAll('#imageNav .scenePreview').forEach(function(scenePreview) {
        ...
        if (currentScene) {
            if (dtIntroTime < TRANSITION_TIME) {
                currentScene.view.setProjectionToMovement(
                    introYaw,
                    (1 - easeInOutQuad(dtIntroTime / TRANSITION_TIME)) * MOVEMENT_DISTANCE * -1
                );
            } else {
        ...
        if (lastScene) {
            if (dtIntroTime < TRANSITION_TIME) {
                lastScene.view.setProjectionToMovement(
                    outroYaw,
                    easeInOutQuad(dtIntroTime / TRANSITION_TIME) * MOVEMENT_DISTANCE
                );
            } else {
    ...
    function switchScene(scene) {
        var photoOrientations = APP_DATA_PHOTO_ORIENTATIONS[scene.data.id] || { yaw: 0, roll: 0 };

        if (currentScene) {
            var photoOrientationsCurrentScene = APP_DATA_PHOTO_ORIENTATIONS[currentScene.data.id] || {
                yaw: 0,
                roll: 0
            };
            viewTargetParams = {
                pitch: viewTargetParams.pitch,
                yaw: currentScene.view.yaw() - photoOrientationsCurrentScene.yaw + photoOrientations.yaw
            };
            scene.view.setParameters(viewTargetParams);
            lastScene = currentScene;
            introYaw = outroYaw - photoOrientationsCurrentScene.yaw + photoOrientations.yaw;
        } else {
            viewTargetParams = {
                pitch: 0,
    ...
    function yawMinDistance(yaw1, yaw2) {
        var jumpDistance = Math.min(Math.abs(yaw1 - (yaw2 - Math.PI * 2)), Math.abs(yaw1 - (yaw2 + Math.PI * 2)));
    ...
    viewer.domElement().addEventListener('mousemove', function(ev) {
        ...
        var coords = currentScene.view.screenToCoordinates({ x: ev.screenX, y: ev.screenY });
